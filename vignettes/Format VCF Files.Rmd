---
title: "Objects of Class SNVdata"
...
output:
  html_document:
    ...
    toc_depth: 3
vignette: |
  %\VignetteIndexEntry{Objects of Class SNVdata} 
...
In this vignette we demonstrate how users may:

1. import pre-formatted `SNVdata` objects using the `import_SNVdata` function, or
2. re-format data from vcf files to objects of class `SNVdata`.
...
To format the fixed data, including the `INFO` variables, the `vcfR` package offers the `vcfR2tidy` function.  We now demonstrate how to use `vcfR2tidy` to format the fixed data.  For additional information regarding the `vcfR2tidy` function please execute `help(vcfR2tidy)` in the console.
