...
Please note that `SimRVSequences` does NOT allow users to simulate genotypes conditional on phenotype.  Instead, `SimRVSequences` simulates haplotypes conditional on the genotype at a single causal rare variant (cRV), via the following algorithm.
...
In section 3 we demonstrate how the methods provided by `SimRVSequences` can aid in the preparation of the SNV data, and in section 4 we briefly review simulating pedigree data using the `SimRVPedigree` package.

# Prepare Single Nucleotide Variant Data

Before we can simulate genetic data for a sample of pedigrees, we require single-nucleotide variant (SNV) data for the pedigree founders. Several publicly available programs can aid in this task (e.g. msprime and fastsimcoal), however, we have found the Eidos program SLiM [3] to be the most practical for simulation of genome-wide, exon-only SNV data for a large number of unrelated individuals.   
