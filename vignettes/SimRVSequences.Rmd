...
5. `recode_identical`  This is a logical argument that indicates if all SNVs are of the same type, i.e. have the same fitness effects.  When `recode_identical = TRUE` we recode recurrent SNVs at the same locus to a single mutation since SNVs of the same type are identical in every respect, save lineage. By default, `recode_identical = TRUE`.
...
When `TRUE`, the logical argument `recode_identical` indicates that all mutations were simulated with the same fitness effects in SLiM.  
SLiM can model many types of mutations; e.g. SLiM can simulataneously model neutral and deleterious mutations.  When different types of mutations occur at the same locus, carriers will experience different fitness effects depending on the carried mutation.  However, when mutations at the same (locus) location have the same fitness effects, they represent the same mutation from different lineages.  When the argument `recode_identical = TRUE` we recode identical mutations, which are stored separately, to a single mutation.  This convention allows for both reduction in storage and correct estimation of the derived allele frequency of the mutation.  Users who 
