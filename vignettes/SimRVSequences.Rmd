...
For `SimRVpedigree`'s ascertainment process to be valid we require the probability that a random individual carries a risk allele to be small, i.e $\le$ 0.002 [10].
...
Not all of the variables above are required to simulate SNV data; the required variables are: `FamID`, `ID`, `sex`, `dadID`, `momID`, `affected`, `DA1`, and `DA2`.  Please note, if `DA` and `DA2` are not provided, a standard gene drop is performed instead of the condition gene drop described in section XX.
...
From the output above, we see that only families 1 and 4 are segregating the same causal rare variant.  Additionally, we see that family 2 is not segregating any of the SNVs in our pool of causal rare variants.  This action is taken for any fully sporadic pedigrees, which are pedigrees in which all maternally and paternally inherited alleles at the familial disease locus are the wild type.

```{r}
#check to see if pedigree 2 is NOT segregating a cRV
#i.e. check to see if all of the paternally 
#and maternally inherited alleles are 0, which 
#represent the wild-type allele.

#store pedigree 2
ped2 <- study_seq$ped_files[study_seq$ped_files$FamID == 2, ]
#check if all paternally and maternally inherited alleles are 0
all(ped2[, c("DA1", "DA2")] == 0)
```

```{r}
plot(study_seq$ped_files[study_seq$ped_files$FamID == 2, ])
```
