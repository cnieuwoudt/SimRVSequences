...
5. `single_SNVtype`  This is a logical argument that indicates if all SNVs are of the same type, i.e. have the same fitness effects.  When `single_SNVtype = TRUE` we recode recurrent SNVs at the same locus to a single mutation since SNVs of the same type are identical in every respect with the exception of lineage . By default, `single_SNVtype = TRUE`.
...
SLiM can be used to model the formation and transmission of many types of mutations. For example, SLiM can simulataneously model neutral and deleterious mutations.  In SLiM each new mutation is stored as an instance of a mutation object.  When mutations with different fitness effects occur at the same locus, carriers of the mutations will experience different fitness effects.  However, in simulations of a single mutation type different mutations at the same location actually represent the same mutation from different lineages.  When the argument `single_SNVtype = TRUE` we recode these identical mutations to a single mutation.
...
The output `SNV_map` catalogs the SNVs, i.e. columns,  in `ped_haplos`.  For example, the first column in `ped_haplos` is a SNV located on chromosome 1 at position 21568178.  
...
From the output above, we see that when supplied an object of class `famStudy` the `summary` function returns a list containing two items: `fam_allele_count` and `pathway_count`.  The item `fam_allele_count` is a matrix that includes a count of the SNVs shared among the disease-affected relatives in each pedigree.
...
From `pathway_count`, we see that there are several SNVs carried by more than 1 disease-affected study participants; however, only 4 of the SNVs carried by more than one individual reside in the pathway of interest.
