...
Simulation of sequence data for ascertained pedigrees is accomplished with the `sim_RVstudy` function.  We will illustrate the useage of this function in section 5.1.  The `sim_RVstudy` function returns an object of class `famStudy`.  In section 5.2 we discuss objects class `famStudy` in detail.  Finally, in section 5.3 we demonstrate the usage and output of the `summary` function for objects of class `famStudy`. 
...
8. `burn-in` The "burn-in" distance in centiMorgan, as defined by [11], which is required before simulating the location of the first chiasmata with interference. By default, `burn_in = 1000`.
...
From the output above, we see that the `sim_RVstudy` function returns an object of class `famStudy`, which inherits from a `list`.  Specifically, a `famStudy` object is a list that contains the following four items:
...
We will discuss each of the four items contained in the `famStudy` object returned by `sim_RVstudy` in section 5.1.  In section 5.2 we illustrate usage and output of the `summary` function for objects of class `famStudy`.

## Objects of class `famStudy`
As mentioned previously, in this section we describe the four items contained in the `famStudy` object returned by `sim_RVstudy`.
We discuss the `ped_files` data frame returned by `sim_RVstudy` in subsection 5.1.1, the sparse matrix `ped_haplos` in sibsection 5.2.1, the data frame `SNV_map` in subsection 5.3.1, and the data frame `haplo_map` in subsection 5.4.1.
...
Next, we look at the sparse matrix, `ped_haplos`, returned by `sim_RVstudy`.  This sparse matrix contains simulated haplotype data.
```{r}
...
In this subsection we discuss the `SNV_map` data frame returned by `sim_RVstudy`. Recall that one of `sim_RVstudy`'s arguments is also named `SNV_map`.  When the `remove_wild` argument of `sim_RVstudy` is set to `FALSE`, the data frame supplied to `SNV_map` will be returned unchanged.  However, when `remove_wild` is set to `TRUE`, the returned `SNV_map` will be a reduced data frame that contains only SNVs that are carried by at least one study participant. 
...
Next, we look at the `haplo_map` data frame returned by `sim_RVstudy`.  The `haplo_map` data frame is used to map the haplotypes (i.e. rows) in `ped_haplos` to the individuals in `ped_files`.
```{r}
...
To count SNVs shared among affected relatives in each pedigree we use the `count_affectedRV` function.  To use this function we supply the output returned by `sim_RVstudy` to `count_affectedRV`.
...
#view a summary of the study summary
names(study_summary)
```

From the output above we see that the summary function for objects of class `famStudy` returns a list containing two items: `fam_allele_count` and `pathway_count`.  The item `fam_allele_count` is a matrix that includes a count of the SNVs shared among the disease-affected relatives in each pedigree.
...
The item `pathway_count` is a data frame that catalogs the total number of SNVs observed in affected study participants.
...
From pathway_count , we see that there are several SNVs carried by more than 1 disease-affected relative in the study; however, only XXX of the SNVs carried by more than one individual reside in the pathway of interest.
...
Importing genome-wide, exon-only SNV data for 10,000 diploid individuals from the .txt file produced by SLiM's outputfull() method, using the `read_slim` function required approximately 2.7 minutes.  The imported data originally contained a total of 306,825 SNVs over 20,000 haplotypes.
