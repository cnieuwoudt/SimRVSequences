...
Family-based studies are more attractive than case-control studies because they have more power to detect causal rare variants, require smaller sample sizes, and can more accurately detect sequencing errors [12].  However, data collection for these studies is both time consuming and expensive. Furthermore, evaluating methodology to identify causal rare variants requires simulated data. In this vignette we illustrate how the methods provided by `SimRVSequences` may be used to to simulate single-nucleotide variant (SNV) data for family-based studies of diploid organisms.  

Please note that `SimRVSequences` is NOT appropriate for users who wish to (1) simulate genotype conditional on phenotype, or (2) simulate inheritance of multiple **causal rare variants (cRVs)** in the same family.  Instead, `SimRVSequences` employs the following algorithm.

1. For each pedigree, we sample a single familial cRV from a pool of causal SNVs specified by the user (see section 3.3).  The sampled cRV represents the underlying genetic contribution to disease-affection status.
...
3. Proceeding forward in time, from founders to more recent generations, for each parent/offspring pair we    
    ...
    2. perform a conditional gene drop to model inheritance of a cRV (see section 6).
...
The `hg_apopPath` data set is similar the `hg_exons` data set discussed in section 3.1.  However, `hg_apopPath` only catalogs the positions of exons contained in our pseudo-pathway.
...
1. `ped_files` A data frame of pedigrees.  This data frame must contain the variables:  `FamID`, `ID`, `sex`, `dadID`, `momID`, `affected`, `DA1`, and `DA2`.  If `DA` and `DA2` are not provided we assume the pedigrees are fully sporadic.  See section 4 for more details.
...
7. `gamma_params` The respective shape and rate parameters of the gamma distribution used to simulate distance between chiasmata.  By default, `gamma_params = c(2.63, 2*2.63)`, as in [11].
8. `burn-in` The "burn-in" distance in centiMorgan, as defined by [11], which is required to simulate the location of the first chiasmata with interference. By default, `burn_in = 1000`.
...
Comparing the plot above to the plot of the same family in section 4, we can see that the number of relatives for whom we simulate SNV data is far less that the total number of relatives in the ascertained pedigree.
...
In this section we describe the model assumptions employed by `SimRVSequences` to simulate SNV data after all required user-inputs have been obtained. 
...
2. Recall that users are expected to specify a pool of cRVs which may represent cRVs located in a pathway, or different cRVs in the same gene.  The size of the pool is entirely up to the user: it can contain many SNVs or only a single SNV.  We urge users who are using `SimRVPedigree` in conjuntion with `SimRVsequences` to choose cRVs so that their cumulative probability is approximately 0.001 (see section 3.3).  From this pool we sample a single cRV for each pedigree with replacement.  
3. Founder haplotypes are sampled from a user-specified, population distribution of haplotypes, conditional on their cRV status.
    + For the founder who introduces the cRV we sample one haplotype from haplotypes that carry the familial cRV and the other from the haplotypes that do not carry ANY of the cRVs in our pool.  Hence, this founder may introduce only a single copy of the familial cRV.
    + For the pedigree founders who do NOT introduce a cRV we sample their haplotypes from the haplotypes that do not carry ANY of the cRVs in our pool. Hence, fully sporadic families will not segregate any cRVs, nor will any non-carrier, marry-ins in our genetic families.  
...
We use these assumptions in our algoritm to simulate sequence data for pedigrees (see introduction.)

# Method Overview

The methods provided by `SimRVSequences` are described as follows.

* Methods to assist with simulating and importing SLiM [3] data:
  ...
  2. `create_slimMap` This function is used both before and after simulating SLiM data. Prior to simulating the SNV data with SLiM, this function can be used to create the recombination map required by SLiM (see section 3.1). Additionally, if this function was used to create the recombination map users may then supply the output of this function to `read_slim` to remap mutations to their correct locations when importing SLiM data.
  3. `read_slim` This function is used to import the .txt output produced by SLiM's outputfull() method (see section 3.3).
* Methods to assist with simulation and basic investigation of SNV data for pedigrees
  4. `sim_RVstudy` This function is used to simulate the SNV data for a sample of pedigrees (see section 5).
  5.  `count_affectedRV` This function counts how many copies of an SNV are shared among the disease-affected relatives in each pedigree contained in the sample.

# Timing

**All of the following timing estimates were performed on a Windows OS with an i7-4790 @ 3.60GHz and 12 GB of RAM.**

Importing genome-wide, exon-only sequence data from a text file produced by SLiM's outputFull() method (see section 3.2) required approximately 3 minutes.

For timing of the `sim_RVstudy` function, we explore the effects of the `affected_only` and `remove_wild` arguments on a sample of 100 pedigrees each of which contained at least 3 disease-affected relatives.  The 100 pedigrees used to time `sim_RVstudy` were simulated using the same settings in parallel processing example of section 4.  For timing, we used genome-wide, exon-only data for 10,000 indivdiuals simulated by SLiM.  After reducing the data to contain SVNs with derived allele frequency less than or equal to 0.01, there were a total of 178,997 SNVs in our data.  We choose a cRV pool of 20 SNVs each with derived allele frequency of 5e-05 at random from the SNVs located in the pseudo-apoptosis pathway described by the `hg_apopPath` data set.
...
---------|--------|------|--------------
200|TRUE|TRUE|0.976
200|TRUE|FALSE|1.267
200|FALSE|TRUE|2.032
200|FALSE|FALSE|3.051
