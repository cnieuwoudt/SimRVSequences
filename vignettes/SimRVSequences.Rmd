...
4. We use a conditional gene-drop algorithm to model inheritance from parent to offspring, which can be described as follows for each parent-offspring pair.
  ...
  * To determine which of the four gametes is transmitted from parent to offspring, we consider the rare-variant status of the parent and the offspring, as follows.
    + Case 1: If **both the parent and offspring carry the cRV** we randomly select (with equal probability) the inherited gamete from the two possible parental gametes that do carry the causal rare variant.
    + Case 2: If the **parent carries the cRV but the offspring does not**, we randomly select (with equal probability) the inherited gamete from the two possible parental gametes that do not carry the causal rare variant.
    + Case 3: If the **parent is not a carrier of the cRV**, then the cRV status of the offspring is irrelevant for this parent's transmission because the offspring cannot inherit it from this parent.  In this scenario, we randomly select the inherited gamete from the four parental gametes with equal probability.
...
The `EXmut` data frame is used to catalog the SNVs in `EXgen`.  The variable `colID` associates the rows of `EXmut` to the columns of `EXgen`, `chrom` identifies the chromosome number, `position` is the position of the SNV in base pairs, `afreq` is the derived allele frequency of each SNV, and `marker` is a unique identifier for each SNV.
...
dim(EXgen)
```
Looking at the output above we see that there are 20,000 haplotypes which contain 192 mutations in `EXgen`.
...
Consider a pseudo-apoptosis sub-pathway centered about the TNFSF10 gene based on the 25 genes that have the highest interaction with this gene in the UCSC Genome Browser's Gene Interaction Tool [5, 6, 7].  The data for this pseudo-pathway is contained in the `hg_apopPath` data set.
...
Consider a set of $n$ causal SNVs with derived allele frequencies $p_i, p_2, ..., p_n$, and let $p_{tot} = \sum_{i=1}^n p_i$.  For sufficiently rare SNVs, we may compute the cumulative carrier probability as $p_{tot}^2 + 2*p_{tot}(1-p_{tot})$.

To satisfy the condition $p_{carrier} \le 0.002$, we require that $1 - (p{tot} - 1)^2 \le 0.002$, or that $p{tot} \le 0.001$.
...
The output above tabulates variants in our pathway by their derived allele frequency,  For example, our pathway contains 12 SNVs with derived allele frequency 5e-05, 4 SNVs with derived allele frequency 1e-04, 13 SNVs with derived allele frequency 0.00015, and so on.  In total, we can see that there are a variety of SNVs in our pathway to choose from.

To obtain a pool of variants with $p_{tot} \sim 0.001$, we could choose the 12 SNVs with derived allele frequency 5e-05, and 4 SNVs with derived allele frequency 1e-4 to be our causal SNVs, so that $p_{tot} = 12*5e-5 + 4*1e-4 = 0.001$.

```{r}
# Create the variable 'possibleRV', which is TRUE for SNVs
# in our pathway with derived allele frequncy 5e-05 or 1e-04,
# and FALSE otherwise
mutDF$possibleRV = ifelse(mutDF$afreq %in% c(5e-5, 1e-04) & 
                            mutDF$pathwayRV == TRUE,
                          TRUE,
                          FALSE)
...
From the output above we see that we have selected a pool of 16 causal variants in our pathway with a cumulative allele frequency of 0.001. 
...
The columns displayed in the output above are described as follows:
 ...
 7. `DA1`: paternally inherited allele at the assumed disease locus: `DA1 = 1` if the cRV is present, and `0` otherwise.
 8. `DA2`: maternally inherited allele at the assumed disease locus: `DA2 = 1` if the cRV is present, and `0` otherwise.
...
3. `haplos` A sparse matrix of genotype data, which is assumed to be of the same format as `EXgen`.
2. `SNV_map` A data frame cataloging the SNV data in `haplos`. This is assumed to be of the same format as `EXmut`. 
...
5. `gamma_params` The respective shape and rate parameters of the gamma distribution used to simulate distance between chiasmata.  By default, `gamma_params = c(2.63, 2.63/0.5)`, as in [9].
6. `burn-in` The burn in distance in cM, used to simulate first chiasmata location. By default, `burn_in = 1000`, as in [9].
...
From the above output we see that `sim_RVstudy` returns three items: a sparse matrix named `ped_genos`, a data frame named `geno_map`, and a data frame named `SNV_map`.   


```{r}
#View the first 15 haplotypes in ped_genos
study_seq$ped_genos[1:15, ]
```

The `ped_genos` output contains the haplotypes for the  individuals in `ped_files`.  If `affected_only = TRUE`, this matrix will contain data for affected individuals and individuals who connect them.  If `affected_only = FALSE` `ped_genos` contains the haplotype data for all individuals in `ped_files`.

```{r}
#Determine the dimensions of ped_genos
dim(study_seq$ped_genos)

#Determine the dimensions of EXgen
dim(EXgen)
```


Notice also that the number of columns (i.e. mutations) in `ped_genos` is far less than the number of columns in `EXgen`.  This is because with the `remove_wild` setting we remove superfluous markers from our data.  That is, when no one in our study carries a mutated allele at a SNV location, we remove the marker from the data.
