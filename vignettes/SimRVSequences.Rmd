...
In what follows we will discuss usage of the `sim_RVstudy` function in section 5.1, the `object_name` returned by the `sim_RVstudy` function in section 5.2, and methods for objects of class `object_name` in section 5.3. 
...
We will discuss each of the four outputs returned by `sim_RVstudy` in the following subsections.  We discuss the `ped_files` data frame returned by `sim_RVstudy` in subsection 5.1, the sparse matrix `ped_haplos` in sibsection 5.2, the data frame `SNV_map` in subsection 5.3, and the data frame `haplo_map` in subsection 5.4.  Finally, in subsection 5.5 we give a brief overview of the available methods to manipulate the output returned by `sim_RVstudy`.
...
## The `count_affectedRV` and `function_name` functions
...
```{r}
# To calculate the total number of SNVs shared
...
From the output above, we see that there are several SNVs carried by more than 1 disease-affected relative in the study; however, only XXX of the SNVs carried by more than one individual reside in the pathway of interest.
