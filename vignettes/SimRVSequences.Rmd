...
Consider a pseudo-apoptosis sub-pathway centered about the TNFSF10 gene based on the 25 genes that have the highest interaction with this gene in the USCS Genome Browser's Gene Interaction Tool [5, 6, 7].  The data for this pseudo-pathway is contained in the `hg_apopPath` data set.
...
The R package `SimRVPedigree` [3] is used to simulate pedigrees ascertained for multiple disease-affected relatives.  For the purpose of illustration we will use the `SimRVPedigree` data set `EgPeds` to serve as our sample of ascertained pedigrees.  To learn more about simulating pedigrees with `SimRVPedigree` please refer to `SimRVPedigree`'s documentation and vignette.
...
[Alternatively]
A full description of the output above can be found in the documentation and vignette for `SimRVPedigree`.

To simulate sequence data we require the variables: `FamID`, `ID`, `sex`, `dadID`, `momID`, `affected`, `DA1`, and `DA2`.

To learn more about simulating pedigrees with `SimRVPedigree` please refer to `SimRVPedigree`'s documentation and vignette.

The R package `SimRVPedigree` [3] is used to simulate pedigrees ascertained for multiple disease-affected relatives. To illustrate the usage of `SimRVSequences` methods we will simulate a single pedigree using the `sim_RVped` method.
...
# Simulate pedigree ascertained for at least two disese-affected individuals, 
# according to the age-specific hazard rates in the `AgeSpecific_Hazards` data 
# set, ascertained from 1995 to 2015, with seed-founder birth year spanning 
# from 1900 to 1920, stop year set to 2017, and family identification number 
# set to 1, and with genetic relative-risk 20.
set.seed(1960)
library(doParallel)
library(doRNG)
...
# Plot the ascertained pedigree (i.e. after proband selection and trimming)
plot(ex_RVped)

#View the first 4 rows of ex_ped
head(ex_RVped, n = 4)
```
