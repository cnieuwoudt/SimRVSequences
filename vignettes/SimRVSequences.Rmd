...
5. `recode_identical`  This is a logical argument that indicates if identical SNVs should be cataloged a single observation.  By default, `recode_identical = TRUE` so that recurrent SNVs at the same locus are recorded as a single observation.
...
When `TRUE`, the logical argument `recode_identical` indicates that identical SNVs should be recorded as a single observation.  SLiM can model many types of mutations; e.g. neutral, benefical, and deleterious mutations.  When different types of mutations occur at the same locus carriers will experience different fitness effects depending on the carried mutation.  However, when mutations at the same location have the same fitness effects, they represent an identical mutation from different lineages.  Even so, SLiM stores identical mutations separately and calculates their prevalence independently.  When the argument `recode_identical = TRUE` we recode identical mutations to be stored as single mutation and calculate their derived allele frequency based on their combined prevalence.  This convention allows for both reduction in storage and correct estimation of the derived allele frequency of the mutation.  Users who prefer to store identical mutations from independent lineages as unique entries should set `recode_identical = FALSE`.
