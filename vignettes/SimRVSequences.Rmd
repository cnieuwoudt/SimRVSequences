...
Not all of the variables above are required to simulate SNV data; the required variables are: `FamID`, `ID`, `sex`, `dadID`, `momID`, `affected`, `DA1`, and `DA2`.  Please note, if `DA` and `DA2` are not provided, the pedigree is assumed to be fully-sporadic. 
...
1. `ped_files` A data frame of pedigrees.  This data frame must contain the variables:  `FamID`, `ID`, `sex`, `dadID`, `momID`, `affected`, `DA1`, and `DA2`. See section XX for more details.
...
4. `affected_only` A logical argument.  When `affected_only = TRUE`, data is simulated only for affected individuals and the family members that connect them on a line of descent.  When `affected_only = FALSE` SNV data is simulated for the entire pedigree. By default, `affected_only = TRUE`.
...
Notice that `ped_haplos` only has 24 columns (i.e. mutations), while `EXhaps`, which was supplied to `sim_RVstudy`, has a total of 500 columns.  This is because the `remove_wild` setting was used to remove superfluous SNVs from our data.  That is, when no one in the study carries a mutated allele at a SNV locus, that marker is removed from the data.  Since we are focused on rare variation, and since `EVhaps` and `study_peds` are small toy data sets the reduction in data is quite significant.
...
Looking at the output above, we see that only one affected relative in family 1 carries a copy of the first marker, `1_236566020`.  It is noteworthy that families 1 and 4 carry so many copies of SNV `2_201284953`.  Not surprisingly, this is the causal rare variant for these two families.
...
2. Founder haplotypes are sampled from a user-specified, population distribution of haplotypes conditioned on their cRV status.  
    + For founders who introduce the cRV: sample from haplotypes that carry the cRV (if cRVs are common or linked the RV founder may introduce multiple cRVs by chance). (**Test and add warning?**)  
    + For founders who do NOT introduce the cRV: sample from the haplotypes that do not carry ANY of the cRVs in our pool of causal cRVs. Hence, fully sporadic families will not segregate any cRVs, nor will the non-carrier marry-ins in our genetic families.  
