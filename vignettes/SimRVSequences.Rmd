...
**OPTION 1**  
DESCRIBE THE MODEL WE USE... OR IS THIS ASSUMPTIONS??

Please note that `SimRVSequences` is NOT appropriate for users who wish to:  
...
*Remove this next bit altogether?*  
However, one could use other programs in conjunction with `SimRVSequences` to accomplish one or both of these tasks. For example, `SimRVSequences` can be used to perform a standard gene drop (see appendix), and then simulate phenotype given a multilocus genotype with other software (i.e, OpenMendel, RarePedSim).  


**OPTION 2**  
Please note that `SimRVSequences` is NOT appropriate for users who wish to (1) simulate genotype conditional on phenotype, or (2) simulate inheritance of multiple causal rare variants in the same family.  Instead, `SimRVSequences` employs a single causal variant model and conditionally simulates genotype given rare variant status.  Additional information regarding the model and assumptions applied by `SimRVSequences` please refer to section 6. 
...
In section XX we demonstrate how the methods provided by `SimRVSequences` can aide in the preparation of the SNV data, and in section 5 we briefly review simulating pedigree data using the `SimRVPedigree` package.
...
Only three of the variables returned by `create_slimMap` are required by SLiM to simulate exon-only data: `recRate`, `mutRate`, and `endPos`.  The other variables seen in the output above are used by the `read_slim` function to remap mutations to their correct positions when importing SLiM data to `R` (see section XX). 
...
2. `recomb_map`: (Optional) A recombination map of the same format as the data frame returned by `create_slimMap`.  Users who followed the instructions in section XX, may supply the output from `create_slimMap` to this argument. 
...
The `hg_apopPath` data set is similar the `hg_exons` data set discussed in section XX.  However, `hg_apopPath` only catalogs the positions of exons contained in our pseudo-pathway.

The following code demonstrates how the `read_slim` function would be called if `create_slimMap` was used to create the recombination map for SLiM as in section XX.
...
The `EXmuts` data set represents the `Mutations` data frame returned by `read_slim,` and the `EXhaps` data set represents the sparse matrix `Haplotypes` returned by `read_slim`.  We note that these toy data sets contain SNV data from a small portion of the human genome are are not representative of exon-only data over the entire human genome.
...
For `SimRVpedigree`'s ascertainment process to be valid we require that the probability that a random individual carries a risk allele to be small, i.e $\le$ 0.002 [10].
...
Observe from the output above that `EXmuts` now contains the variable `is_CRV`.  The `sim_RVstudy` function (discussed in section XX) will sample a causal variant for each family in the study from the SNVs for which `is_CRV = TRUE`, so that different families may segregate different rare variants. Upon identifying the familial cRV we then sample haplotypes for each founder from the distribution of haplotypes conditioned on the founder's cRV status.  This ensures that the cRV is introduced by the correct founder.
 

# Prepare Pedigree Data <a name="PedSample"></a>

The R package `SimRVPedigree` [6] is used to simulate pedigrees ascertained for multiple disease-affected relatives.  The following example demonstrates how users could use `SimRVPedigree` to simulate five pedigrees ascertained for three or more disease-affected relatives in parallel with the `doParallel`[11] and `doRNG`[12] packages.  
...
Not all of the variables above are required to simulate SNV data; the required variables are: `FamID`, `ID`, `sex`, `dadID`, `momID`, `affected`, `DA1`, and `DA2`.  Please note, if `DA` and `DA2` are not provided, the pedigree is assumed to be fully-sporadic, i.e. `DA1 = DA2 = 0` for all members. 
...
Simulating SNV data for a sample of ascertained pedigrees is achieved with `sim_RVstudy`.  The `sim_RVstudy` function has the following required and optional arguments:

Required Arguments:

1. `ped_files` A data frame of pedigrees.  This data frame must contain the variables:  `FamID`, `ID`, `sex`, `dadID`, `momID`, `affected`, `DA1`, and `DA2`.  If `DA` and `DA2` are not provided we assume the pedigrees are fully sporadic.  See section XX for more details.
...
3. `SNV_map` The rows of the data frame `SNV_map` provide information on the SNVs in `haplos.`  If `read_slim` was used to import data, the data frame `Mutations` returned by this function is of the proper format (see section XX).  We note that users must add the variable `is_CRV` to this data frame, as discussed in section XX.

Optional Arguments:

4. `affected_only` A logical argument.  When `affected_only = TRUE` we only simulate SNV data for the affected individuals and the family members that connect them on a line of descent.  When `affected_only = FALSE` SNV data is simulated for the entire pedigree. By default, `affected_only = TRUE`.
...
6.  `pos_in_bp` is a logical variable which indicates if the positions in `SNV_map` are listed in base pairs.  By default, `pos_in_bp = TRUE`. If the positions in `SNV_map` are listed in centiMorgan set `pos_in_bp = FALSE` instead.
...
8. `burn-in` The "burn-in" distance in centiMorgan, as defined by [8], which is required to simulate the location of the first chiasmata with interference. By default, `burn_in = 1000`.

Assuming readers have followed the steps outlined in sections XX and XX, we may execute the `sim_RVstudy` function as follows.
...
#Since we used affected_only option the pedigree has been  
#reduced to contain only disease-affected relatives.
plot(study_seq$ped_files[study_seq$ped_files$FamID == 3, ],
     location = "bottomleft")
```

Comparing the plot above to the plot of the same family in section XX, we can see that the number of relatives for whom we simulate SNV data is far less that the total number of relatives in the ascertained pedigree.
...
Notice that `ped_haplos` only has 25 columns (i.e. mutations), while `EXhaps`, which was supplied to `sim_RVstudy`, has a total of 500 columns.  This occurs when the `remove_wild` setting is used to remove superfluous SNVs from our data.  That is, when no one in the study carries a mutated allele at a SNV locus, that marker is removed from the data.  Since we are focused on rare variation, and since `EXhaps` and `study_peds` are small toy data sets the reduction in data is quite significant.
...
From the output above, we see that families 1 and 4 segregate variant `2_201284953`, family 2 does not segregate any SNV in the pool of causal rare variants, family 3 segregates variant `10_89016102`,  and family 5 segregates variant `10_89015798`.   Family 2 does not segregate any of the SNVs in our pool of causal rare variants because it is a fully sporadic pedigree, i.e. there is no genetic predisposition to disease in this pedigree.

To count the total number of SNVs shared among affected relatives in each pedigree we use the `count_affectedRV` function.  To use this function we supply the output returned by `sim_RVstudy` to `count_affectedRV`.
...
Looking at the output above, we see that only one affected relative in family 1 carries a copy of the first marker, `1_236566020`.  It is noteworthy that the disease-affected relative in families 1 and 4 carry so many copies of SNV `2_201284953`.  Not too surprisingly, this is the causal rare variant for these two families.

```{r}
# To caluculate the total number of SNVs shared
# by affected relatives across all families we 
# use colSums, from the Matrix package
library(Matrix)
count_SNVshare <- colSums(count_affectedRV(study_seq)[, -1])
count_SNVshare
```

From the output above, we see that there are several SNVs carried by more than 1 disease-affected relative in the study. 

# `SimRVSequences` Model Assumptions

In this section we describe the model assumptions employed by `SimRVSequences` and outline the algorithm used to simulate SNV data after all required user-inputs have been obtained. 

## Assumptions 
1. Given a sample of ascertained pedigrees we allow families to segregate different rare variants, but assume that within a family genetic cases are due to a single, **causal rare variant (cRV)** that increases disease susceptibility.  We assume that only one copy of the familial cRV is introduced to the pedigree and that the cRV status of every pedigree member is known.  The `R` package `SimRVPedigree` [6] may be used to simulate pedigrees that meet these criteria (see section XX).  
2. Founder haplotypes are sampled from a user-specified, population distribution of haplotypes conditioned on their cRV status.
    + For the founder who introduces the cRV we sample one haplotype from haplotypes that carry the familial cRV and the other from the haplotypes that do not carry ANY of the cRVs in our pool of causal cRVs.  Hence, this founder may introduce only a single copy of the familial cRV.
    + For the pedigree founders who do NOT introduce a cRV we sample their haplotypes from the haplotypes that do not carry ANY of the cRVs in our pool of causal cRVs. Hence, fully sporadic families will not segregate any cRVs, nor will any non-carrier, marry-ins in our genetic families.  
...
## Algorithm to simulate SNV data for a pedigree

Algorithm to Simulate Pedigree Data

1. Sample a familial cRV from the causal SNV pool specified by the user (see section XX).
2. Draw pedigree founder haplotypes from SNV data conditioned on the founder's cRV status at the familial disease locus.
3. (top-to-bottom/forwards-in-time approach) For each parent/offspring pair:
    1. Simulate recombination and formation of parental gametes
    2. perform conditional gene drop (described in assumptions)
3. (top-to-bottom/forwards-in-time approach) For each parent/offspring pair:
    1. simulate locations future chiasmata 
    2. sample chromatids to participate in crossover event
    3. simulate formation of gametes 
    4. perform conditional gene drop (described in assumptions) 

# Timing

**All of the following timing estimates were performed on a Windows OS with an i7-4790 @ 3.60GHz and 12 GB of RAM.**

Importing exon-only sequence data from the text file produced by SLiM's outputFull() method (see section XX) required approximately 3 minutes.
...
[10] Christina Nieuwoudt, Samantha J Jones, Angela Brooks-Wilson, and Jinko Graham (2018). 
...
[11] Microsoft Corporation and Steve Weston (2017).
  **doParallel: Foreach Parallel Adaptor for the 'parallel' Package.** 
  *R package version 1.0.11.*
  https://CRAN.R-project.org/package=doParallel.
  
[12] Renaud Gaujoux (2017). 
