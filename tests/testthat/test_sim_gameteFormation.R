...
test_that("sim_gameteFormation returns 2 lists each with the same length as the number of rows in chrom_map", {
  ...
  my_chrom_map <- data.frame(start = round(runif(n_chroms, 0, 100)))
  ...
  my_gams <- sim_gameteFormation(my_chrom_map, allele_IDs = c(0, 1))
  length(my_gams[[1]])

  expect_equal(length(my_gams[[1]]), nrow(chrom_map))
  expect_equal(length(my_gams[[2]]), nrow(chrom_map))
})
